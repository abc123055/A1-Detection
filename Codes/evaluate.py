...
def _find_best_f1_threshold(all_labels, all_scores):
    """Find PSNR threshold that maximizes F1 for abnormal class (label=1)."""
    anomaly_scores = 1.0 - all_scores
    prec_arr, rec_arr, thresholds = metrics.precision_recall_curve(
        all_labels, anomaly_scores, pos_label=1)
    with np.errstate(invalid='ignore'):
        f1_arr = np.where((prec_arr + rec_arr) > 0,
                          2 * prec_arr * rec_arr / (prec_arr + rec_arr), 0.0)
    best_idx = np.argmax(f1_arr[:-1])
    return float(1.0 - thresholds[best_idx]), float(f1_arr[best_idx])


def _normalize_records(psnr_records):
    norm = []
    for d in psnr_records:
        d = d.copy()
        if NORMALIZE:
            d -= d.min()
            d /= d.max()
...
def compute_frame_accuracy(loss_file):
    SEP = '=' * 50

    if not os.path.isdir(loss_file):
        loss_file_list = [loss_file]
    else:
        loss_file_list = sorted([os.path.join(loss_file, f) for f in os.listdir(loss_file)])
    ...
    for sub_loss_file in loss_file_list:
        ...
        global_min = min(d.min() for d in psnr_records)
        global_max = max(d.max() for d in psnr_records)
        norm_records = [(d - global_min) / (global_max - global_min) for d in psnr_records]

        all_scores = np.concatenate([d[DECIDABLE_IDX:] for d in norm_records])
        all_labels = np.concatenate([gt[i][DECIDABLE_IDX:] for i in range(num_videos)])

        best_thresh, best_f1 = _find_best_f1_threshold(all_labels, all_scores)
        all_preds = (all_scores < best_thresh).astype(np.int8)
        acc, prec, rec, f1 = _compute_metrics(all_labels, all_preds)

        # output directory
        out_dir = os.path.join(os.path.dirname(sub_loss_file), 'results')
        if not os.path.exists(out_dir):
            os.makedirs(out_dir)

        # video names
        frames_dir = GroundTruthLoader.NAME_FRAMES_MAPPING.get(dataset, '')
        if os.path.isdir(frames_dir):
            video_names = sorted(os.listdir(frames_dir))
        else:
            video_names = ['{:03d}'.format(i + 1) for i in range(num_videos)]

        lines = []
        lines.append('=== Frame-level evaluation (global normalization) ===\n')
        lines.append(SEP)
        lines.append('Threshold      : {:.6f}  [optimal (best F1={:.4f})]'.format(best_thresh, best_f1))
        lines.append('Accuracy       : {:.4f}'.format(acc))
        lines.append('Precision      : {:.4f}'.format(prec))
        lines.append('Recall         : {:.4f}'.format(rec))
        lines.append('F1             : {:.4f}'.format(f1))
        lines.append(SEP)
        lines.append('')

        for i in range(num_videos):
            frame_scores = norm_records[i][DECIDABLE_IDX:]
            frame_labels = gt[i][DECIDABLE_IDX:]
            frame_preds  = (frame_scores < best_thresh).astype(np.int8)
            v_acc, _, _, v_f1 = _compute_metrics(frame_labels, frame_preds)
            wrong = int(np.sum(frame_preds != frame_labels))
            total = len(frame_labels)
            vname = video_names[i] if i < len(video_names) else '{:03d}'.format(i + 1)
            csv_name = '{}_frame_eval.csv'.format(vname)

            with open(os.path.join(out_dir, csv_name), 'w') as f:
                f.write('frame_id,gt,pred,score,correct\n')
                for j, (s, lb, pd) in enumerate(zip(frame_scores, frame_labels, frame_preds)):
                    f.write('{},{},{},{:.6f},{}\n'.format(
                        j + DECIDABLE_IDX, int(lb), int(pd), float(s), int(lb == pd)))

            lines.append('  {}: acc={:.3f}  f1={:.3f}  wrong={}/{}  -> {}'.format(
                vname, v_acc, v_f1, wrong, total, csv_name))

        summary_path = os.path.join(out_dir, 'frame_eval_summary.txt')
        lines.append('')
...
def compute_video_accuracy(loss_file):
    SEP = '=' * 50
    ...
    for sub_loss_file in loss_file_list:
        dataset, psnr_records, gt = load_psnr_gt(loss_file=sub_loss_file)
        norm_records = _normalize_records(psnr_records)
        num_videos   = len(psnr_records)

        all_scores = np.concatenate([d[DECIDABLE_IDX:] for d in norm_records])
        all_labels = np.concatenate([gt[i][DECIDABLE_IDX:] for i in range(num_videos)])
        best_thresh, _ = _find_best_f1_threshold(all_labels, all_scores)

        video_preds  = np.zeros(num_videos, dtype=np.int8)
        video_labels = np.zeros(num_videos, dtype=np.int8)
        video_max_errors = np.zeros(num_videos, dtype=np.float32)
        for i in range(num_videos):
            frame_scores = norm_records[i][DECIDABLE_IDX:]
            frame_preds  = (frame_scores < best_thresh).astype(np.int8)
            video_preds[i]      = int(np.any(frame_preds == 1))
            video_labels[i]     = int(np.any(gt[i][DECIDABLE_IDX:] == 1))
            video_max_errors[i] = float(1.0 - frame_scores.max())  # max anomaly score

        acc, prec, rec, f1 = _compute_metrics(video_labels, video_preds)
        correct_count = int(np.sum(video_preds == video_labels))
        ...
        lines = []
        lines.append('=== Video-level evaluation (threshold={:.6f}) ===\n'.format(best_thresh))
        lines.append(SEP)
        lines.append('Threshold      : {:.6f}  [from frame-level eval]'.format(best_thresh))
        lines.append('Accuracy       : {:.4f}  ({}/{} videos correct)'.format(
            acc, correct_count, num_videos))
        lines.append('Precision      : {:.4f}'.format(prec))
        lines.append('Recall         : {:.4f}'.format(rec))
        lines.append('F1             : {:.4f}'.format(f1))
        lines.append(SEP)
        ...
        csv_rows = ['video,gt,pred,max_error,correct']
        for i in range(num_videos):
            vname  = video_names[i] if i < len(video_names) else '{:03d}'.format(i + 1)
            gt_str = 'anomaly' if video_labels[i] == 1 else 'normal'
            pd_str = 'anomaly' if video_preds[i]  == 1 else 'normal'
            ok     = 'correct' if video_preds[i] == video_labels[i] else 'WRONG'
            lines.append('  {}: gt={:<8s}  pred={:<8s}  max_error={:.4f}  {}'.format(
                vname, gt_str, pd_str, video_max_errors[i], ok))
            csv_rows.append('{},{},{},{:.6f},{}'.format(
                vname, gt_str, pd_str, video_max_errors[i], ok))

        summary_path = os.path.join(out_dir, 'video_eval_summary.txt')
        csv_path     = os.path.join(out_dir, 'video_eval.csv')
        lines.append('')
        lines.append('Summary -> {}'.format(summary_path))
        lines.append('CSV     -> {}'.format(csv_path))
